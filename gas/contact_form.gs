// ─── Lamla AI — Contact Form GAS Script ──────────────────────────────────────
//
// Deploy as a Web App:
//   Extensions → Apps Script → Deploy → New deployment → Web app
//   Execute as: Me | Who has access: Anyone
//
// After deploying, copy the Web App URL and set it as:
//   Frontend env var: REACT_APP_GAS_CONTACT_URL=<your-web-app-url>
//
// This script:
//   1. Appends form submissions to a Google Sheet (auto-created if missing)
//   2. Sends a notification email to the admin
//   3. Sends an acknowledgment email to the user
// ─────────────────────────────────────────────────────────────────────────────

// ── Config — update these before deploying ───────────────────────────────────
var ADMIN_EMAIL = "contact.lamla1@gmail.com";
var SHEET_NAME  = "Contact Form";
...
function sendAdminNotification(name, email, title, message) {
  ...
  GmailApp.sendEmail(ADMIN_EMAIL, subject, stripHtml(html), { htmlBody: html });
}
...
function sendUserAcknowledgment(name, email, title) {
  ...
  GmailApp.sendEmail(email, subject, stripHtml(html), { htmlBody: html });
}
