                    ...
                    {anonymousTutorMessages.map((item, idx) => (
                      <div className="db-rating-item" key={`${item.id || idx}-${item.created_at || idx}`}>
                        <div style={{ width: '100%' }}>
                          ...
                          <span className="db-rating-time">{formatAbsoluteTime(item.created_at)} ({formatRelativeTime(item.created_at)}) • {item.path}</span>
                          <p style={{ marginTop: 6, marginBottom: 4, color: 'var(--text-secondary)', fontSize: '0.82rem' }}>User message</p>
                          <p style={{ marginTop: 0, marginBottom: 8, color: 'var(--text-primary)', whiteSpace: 'pre-wrap' }}>
                            {item.tutor_message}
                          </p>
                          <p style={{ marginTop: 0, marginBottom: 4, color: 'var(--text-secondary)', fontSize: '0.82rem' }}>Tutor response</p>
                          {item.tutor_response ? (
                            <RichTextRenderer
                              text={item.tutor_response}
                              className="db-anon-response-richtext"
                            />
                          ) : (
